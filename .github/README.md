...
|----------|---------|--------|
| `atmos-pro.yaml` | Pull request, merged PR, merge queue | Build image → Run tests → broad PR/merged PR `describe affected --upload`; merge queue `describe affected --stack dev --upload` → Atmos Pro applies dev only |
...
2. Click "Merge when ready" → merge queue runs build/test/`describe affected --stack dev --upload` on the queue commit → Atmos Pro dispatches apply for dev → check-suite status gates the queue → on success, fast-forward to `main` and run post-merge affected-stack plans for visibility
